...
$checkArticle = mysql_query("SELECT * FROM articles WHERE article_id = '$article_id' AND article_start <= '$nowDateTime'");
if (mysql_num_rows($checkArticle)) {
    $article_info = mysql_fetch_assoc($checkArticle);
    $article_title = $article_info['article_name'].' | Статьи free-cheese.com';
    $article_description = strip_tags($article_info['article_text']);
    $article_description = substr($article_description, 0, 300);
    $article_description = '<meta name="description" content="'.$article_description.'">';
}
else {
    header('location: articles');
    exit();
    ...
    <div class="container">
        ...
        <div class="body">
            ...
            if (mysql_num_rows($checkArticle)) {
                ...
                $article_adr = "document.location.assign('oneArticle?id=".$row['article_id']."')";
                if ($user_info['user_nick'] != '') {
                    $article_user_label = $user_info['user_nick'];
                }
                else {
                    $article_user_label = $user_info['user_name'];
                }
                $article_date = date_create($article_info['article_start']);
                ...
                if ($real_user_info['user_privilege'] == '1') {
                    $redBut = '<button class="rewrite" onclick="changeText(this)"><input type="hidden" value="'.$article_info['article_id'].'"></button>';
                }
                echo '
                <div class="colorHeader redLight">
                <h1>'.$article_info['article_name'].'</h1>
            </div>
            <div class="bodyLeft">
                <div class="mainRaffle">
                    <span class="articleContent aboutUs" id="articleContent">'.$redBut.$article_info['article_text'].'<input type="hidden" id="articleID" value="'.$article_info['article_id'].'"></span>
                    <div class="raffleBottom redLight">
                        <div class="raffleBottomContent">
                            <span>Статья от '.$article_date.' (<a href="user?id='.$article_info['user_id'].'">'.$article_user_label.'</a>)</span>
                        </div>
                    </div>
                </div>
