...
<head>
    <meta charset='utf-8'>
    <meta name="description" content="На free-cheese.com вы можете выигрывать призы и подарки, приняв участие в наших розыгрышах. Получить подарок может каждый, абсолютно бесплатно. Так же у нас всегда интересные и свежие новости, заходите и выигрывайте, бесплатный сыр только у нас.">
    ...
    <title>Розыгрыши, призы и приятные подарки от free-cheese.com. Бесплатный сыр только у нас.</title>
    ...
    <div class="container">
        ...
        <div class="body">
            <div class="bodyLeft">
                    ...
                    if (mysql_num_rows($checkMain)) {
                        while ($row = mysql_fetch_assoc($checkMain)) {
                            if($nowDate >= $row['event_start'] && $nowDate <= $row['event_end']) {
                                ...
                                $ruffle_adr = "document.location.assign('mainRaffle?id=".$ruffle_id."')";
                                $ruffle_img = $row['event_avatar'];
                                $ruffle_name = $row['event_name'];
                                echo '<div class="mainRaffle">
                                        <div class="raffleTop">
                                            <div class="raffleTopContent">
                                                <span><a href="mainRaffle?id='.$ruffle_id.'">Основной розыгрыш</a></span>
                                                <button onclick="'.$ruffle_adr.'">Подробнее</button>
                                            </div>
                                        </div>
                                        <a href="mainRaffle?id='.$ruffle_id.'"><img src="'.$ruffle_img.'" alt="Основной розыгрыш"></a>
                                        <div class="raffleBottom">
                                            <div class="raffleBottomContent">
                                                <span><a href="mainRaffle?id='.$ruffle_id.'">'.$ruffle_name.'</a></span>
                                            </div>
                                        </div>
                                    </div>';
                            }
                ...
                <div class="aboutUs">
                    <?php
                    $checkContent = mysql_query("SELECT * FROM contents WHERE content_page = 'index'");
                ...
                <?php
                $checkComments = mysql_query("SELECT * FROM comments WHERE comment_page = '$justLocation' AND comment_answer = '0' ORDER BY comment_date ASC");
                ...
                echo $commentBlock.$commentCreate;
                ?>
            </div>
            <div class="bodyRight">
                <?php
                $checkOther = mysql_query("SELECT * FROM events WHERE event_start <= '$nowDate' AND event_type = '0'");
                ...
                if (mysql_num_rows($checkNews)) {
                    while ($row = mysql_fetch_assoc($checkNews)) {
                        $news_adr = "document.location.assign('oneNews?id=".$row['news_id']."')";
                        $text = strip_tags($row['news_text']);
                        echo '<div class="oneNews">
                                    <div class="newsTop">
                                        <div class="newsTopContent">
                                            <span class="raffleShortHeader"><a href="oneNews?id='.$row['news_id'].'">'.$row["news_name"].'</a></span>
                                            <button onclick="'.$news_adr.'">Подробнее</button>
                                        </div>
                                    </div>
                                    <a href="oneNews?id='.$row['news_id'].'"><img src="'.$row['news_avatar'].'" alt="Новости"></a>
...
<script src="scripts/readUs.js"></script>
...
<!-- toodoo-key: CkZ2fsrTmX5qQAqFu4CjH -->
</body>
</html>